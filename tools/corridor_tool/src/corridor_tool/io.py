...
def _write_array(path: Path,a: np.ndarray):
    ...
    path.with_suffix(path.suffix+".json").write_text(json.dumps(hdr,indent=2),encoding="utf-8")
...
def write_result(result: CorridorResult,out: Path,policy:dict,manifest:dict):
    ...
    result.nodes.to_csv(out/"corridor_nodes.csv",index=False)
    result.edges.to_csv(out/"corridor_edges.csv",index=False)
    point_rows=[]
    ...
    import pandas as pd
    pd.DataFrame(point_rows,columns=["edge_id","point_order","row","col"]).to_csv(out/"corridor_edge_points.csv",index=False)
